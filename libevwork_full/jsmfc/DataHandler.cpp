...
int CDataHandler::onData(evwork::IConn* pConn, const char* pData, size_t uSize)
{
	...
	while (uSize > 0)
	{
		if (uSize < JS_HEADER_SIZE)
			break;

		uint32_t uPktLen = Request::peekLen(pData);

		if ( (uPktLen < JS_HEADER_SIZE) || (m_uPacketLimit != (uint32_t)-1 && uPktLen > m_uPacketLimit) )
		{
			std::string strPeerIp = "";
			uint16_t uPeerPort = 0;
			pConn->getPeerInfo(strPeerIp, uPeerPort);

			LOG(Warn, "[js::CDataHandler::%s] from:%s:%u recv a invalid packet, pktlen:%u", __FUNCTION__, strPeerIp.c_str(), uPeerPort, uPktLen);
...
void CDataHandler::__requestDispatch(Request& request, evwork::IConn* pConn)
{
	...
	{
		LOG(Error, "[js::CDataHandler::%s] catch exeception : %s", __FUNCTION__, e.what());
	}
}

bool CDataHandler::__onHandlerPrint()
{
	LOG(Notice, "[js::CDataHandler::%s] proc:%u bytes:%llu", __FUNCTION__, m_uProc, m_uBytes64);
