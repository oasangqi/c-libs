...
{
	LOG(Notice, "[pb::CThreadWorker::%s] tid:%u running...", __FUNCTION__, syscall(__NR_gettid));
	...
	LOG(Notice, "[pb::CThreadWorker::%s] tid:%u exit!", __FUNCTION__, syscall(__NR_gettid));
}
...
void CAsyncDataHandler::__requestDispatch(Request& request, evwork::IConn* pConn)
{
	...
	{
		LOG(Error, "[pb::CAsyncDataHandler::%s] catch exeception : %s", __FUNCTION__, e.what());
	}
}
