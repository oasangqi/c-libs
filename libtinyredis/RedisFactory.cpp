...
CRedisMap::CRedisMap(const VEC_REDIS_PARAM_t& vecParam)
{
	for (VEC_REDIS_PARAM_t::const_iterator c_iter = vecParam.begin(); c_iter != vecParam.end(); ++c_iter)
	{
		...
		if (c_iter->m_strIp.size() > 0) {
			pNew = new CRedisClient(c_iter->m_strIp, c_iter->m_uPort16, c_iter->m_strPass, c_iter->m_uMiniSeconds);
		} else if (c_iter->m_strPath.size() > 0) {
			pNew = new CRedisClient(c_iter->m_strPath, c_iter->m_strPass, c_iter->m_uMiniSeconds);
		}
...
void CRedisFactory::addRedis(const std::string& strPath, const std::string& strPass, uint32_t uMiniSeconds)
{
	m_vecRedisParam.push_back( SRedisParam(strPath, strPass, uMiniSeconds) );
}
