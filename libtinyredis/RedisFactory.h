...
namespace tinyredis
{
	struct SRedisParam
	{
		...
		SRedisParam(const std::string& strPath, const std::string& strPass, uint32_t uMiniSeconds)
			: m_strPath(strPath), m_strPass(strPass), m_uMiniSeconds(uMiniSeconds)
		{
		}
	};
	...
	public:
		...
		void addRedis(const std::string& strPath, const std::string& strPass, uint32_t uMiniSeconds);
